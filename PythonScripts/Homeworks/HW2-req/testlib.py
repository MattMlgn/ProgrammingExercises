...
class UnderstandingError(Exception):
    pass   # errore nelle domande di comprensione del testo


class ForbiddenError(Exception):
    pass   # errore nell'import o nell'aprire un file proibito


class TimeoutError(Exception):
    pass   # errore di timeout


class TestCase(unittest.TestCase):
    # feedbacks for text comprehension questions
    FEEDBACKS = {}
    ...
    def _raise_forbidden(self, forbidden):
        ...
        raise ForbiddenError(
            f"The usage of the '{forbidden}' function/method is forbidden!")
    ...
    def check_imports(self, allowed=None, forbidden=None):
        ...
        def _check_import(*args, **kargs):
            ...
            if name in forbidden or (not forbidden and name not in allowed):
                print(
                    f"Importing {name} (globals, locals, {rest[-2:]}) (not allowed)")
        ...
        def _check_open(*args, **kargs):
            ...
            for fn, m in allowed.items():
                ...
                if filename.endswith(fn):
                    if not mode in m:
                        print(
                            f"Opening file '{filename}' with mode '{mode}' is not allowed!")
                        raise ForbiddenError(
                            f"Opening file '{filename}' with mode='{mode}' is forbidden!")
                    else:
                        break
            else:
                print(
        ...
        for question, feedbacks in self.FEEDBACKS.items():
            ...
            if answer is None:
                raise UnderstandingError(
                    f"Non hai risposto alla domanda '{question}'")
            if isinstance(answer, str):
                answer = answer.lower()
            correct = None if corrects is None else corrects.get(
                question, None)
            if correct is None:
                raise UnderstandingError(
                    f"TESTERROR: il docente non ha indicato la risposta giusta alla domanda '{question}'")
            for answers, feedback in feedbacks.items():
                if answer in answers:
                    break
            else:
                raise UnderstandingError(
                    f"Non capisco la tua risposta '{answer}' alla domanda '{question}'")
...
Peccato! {feedback}""")
    ...
    def print_timing(self, G, args, T=5, N=10, M=5):
        ...
        imple = min(timeit('implementation' + args, number=X, globals=G)
                    for _ in range(M))    # min time over M run
        ref = min(timeit('reference' + args, number=X, globals=G)
                  for _ in range(M))    # min time over M run
        ratio = imple/ref
    ...
    def check_max_ciclomatic_complexity(_self, file='program01.py', level=10):
        ...
        with open(file, encoding='utf8') as F:
            text = F.read()
        # FIXME: we assume at least one function is there
        worst = sorted_results(cc_visit(text))[0]
        cc = worst.complexity
        rank = cc_rank(cc)
...
Please implement your algorithm by using smaller functions/methods.""")
    ...
    def randomized_symbol(self, module, symbol):
        ...
        if DEBUG:
            print(module, symbol, ' -> ', random_name)
        ...
        try:
            ...
            if DEBUG:
                print(module, random_name, ' -> ', symbol)
    ...
    def check_img_file(self, a, b):
        ...
        if sha1(f.read()).hexdigest() != sha1(g.read()).hexdigest():
            ...
            self.assertEqual(
                wa, wb, f"Images have different widths ({wa} != {wb})")
            self.assertEqual(
                ha, hb, f"Images have different heights ({ha} != {hb})")
            for y in range(ha):
                for x in range(wa):
    ...
    def check_json_file_to_list(self, jsonf, result, msg=(f'\n{"*"*100}\n README: The list of all generated images is '
        ...
        expected_s = tuple(tuple(tuple(c for c in row)
                           for row in mat) for mat in expected)
